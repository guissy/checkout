...
import paymentServiceManager, { PaymentEventData } from "@/machines/payService";
...
export async function GET(request: Request) {
  try {
    ...
    // 发送初始状态
    writer.write(encoder.encode(`data: ${JSON.stringify(PaymentStatus.PENDING)}\n\n`));
      ...
      try {
        while (true) {
          ...
          const newData = await Promise.race<PaymentEventData | null>([
            new Promise((resolve) => 
              paymentServiceManager.once("stateChange", (newData) => {
          ...
          // 发送更新或保持当前状态
          writer.write(
            encoder.encode(`data: ${JSON.stringify(newData?.currentState ?? PaymentStatus.PENDING)}\n\n`)
          );
        }
      } catch (error) {
        // 处理错误
    ...
    return errorResponse(
      (error as Error)?.message || "流处理时发生错误",
      500
    );
  }
} 