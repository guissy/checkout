...
const CurrencyInline: React.FC<Props> = ({ currencyInfo, onOpen, value, onValueChange, disabled }) => {
  ...
  const index = currencyInfo?.findIndex((currency) => currency.currencyCode === value);
  if (total > 0 && total > showCount && Array.isArray(currencyInfo) && index! >= showCount) {
    currencyInfo.splice(showCount - 1, 1, currencyInfo[index!]);
  }
  return (
    <div className={clsx('mt-2 flex space-x-2 *:rounded-full *:text-xs/[1]')} ref={ref}>
      {
        Array.isArray(currencyInfo) && currencyInfo.slice(0, showCount).map((currency) => (
          <div
            key={currency.currencyCode}
            title={i18n.locale === 'zh' ? currency?.currencyName : currency?.currencyNameEn}
            className={clsx(
              ...
              value === currency.currencyCode ? "bg-[#1A1919] text-white" : "hover:bg-[#1A1919]/10"
            )}
          >
            <label className={clsx(
              ...
              <input
                ...
                onChange={() => {
                  ...
                  void reportEvent('currency_change', { currencyCode: currency.currencyCode });
                }}
      ...
      {total > 0 && total > showCount && <More alt="More"
           className={clsx("cursor-pointer hover:opacity-75 order-[2000]",
