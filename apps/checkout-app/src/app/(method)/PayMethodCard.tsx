...
const PayMethodCard: React.FC<PropsWithChildren<Props>> = ({
  ...
  return item.type ? (
        ...
        <details ref={detailsRef}>
          ...
          >
            <div
              ...
              onClick={(e) => {
                const otherToBankTransfer =
                  currentPay?.transactionType !==
                    TransactionType.bankTransfer &&
                  item?.transactionType === TransactionType.bankTransfer;
                const isOpen = detailsRef.current?.open;
                const isClickMore = e.detail === 9527;
                // const isClickCard = e.detail === 9981;
                if (
                  (otherToBankTransfer && isOpen) ||
                  (!isClickMore && isOpen)
                ) {
                  e.stopPropagation();
                  e.preventDefault();
            ...
            >
              <div
                className={clsx(
                  "flex items-center space-x-2 justify-between px-4 sm:px-6",
              ...
              >
                  ...
                  {getIcon(item.type, item.platformName) ? (
                    <div
                      className={clsx(
                        ...
                        noShadow
                          ? ""
                          : "rounded-full overflow-hidden shadow-[0_0_6px_rgba(102,145,231,0.12)]",
                        !noShadow &&
                          bgColor !== "#fff" &&
                          "shadow-[0_1px_6px_rgba(102,145,231,0.30)]!",
                    ...
                    >
                      <Image
                        src={getIcon(item.type, item.platformName)?.[0]}
                        alt=""
                      />
                    </div>
                  ) : (
                ...
                <div
                  className={clsx(
                    isAli ? "w-[calc(100%-146px)]" : "w-[calc(100%-72px)]",
                  )}
                  ref={ref}
                >
                  <h3
                    className="font-bold text-sm/[1] sm:text-base/[1] w-full truncate"
                    title={item.platformName}
                  >
                    {item.platformName}
                  </h3>
                  <p className="text-xs/[1] sm:text-sm/[1] mt-2 text-[#BAB9C1]">
                    {getLifeStagePhaseCN(i18n)[item.transactionType!] ||
                  ...
                  >
                    {item.cards?.slice(0, showCount).map((card, i) => (
                      <div
                        ...
                        data-state={
                          currentPay?.type === card.type ? "active" : ""
                        }
                        onClick={(e) => {
                          const isOpen = detailsRef.current?.open;
                          if (isOpen) {
                    ...
                    {total > 0 && (
                      <MoreLite
                        ...
                        alt=""
                        className={clsx(
                          "cursor-pointer w-[32px] h-[22px] grid place-items-center border border-[#D5E3F2] rounded-[3px]",
                        )}
                      />
                    )}
                  </picture>
                ...
                <div className="flex items-center justify-end">
                  ...
                  {/*</div>*/}
                  <label className={"hidden"} htmlFor={item.type}>
                    {item.platformName}
                  </label>
                  <input
                    type="radio"
                      ...
                      >
                        ...
                        {currentPay?.type === card.type && (
                          <svg
                            ...
                            className={
                              "absolute right-1 top-1 animate-in fade-in duration-300"
                            }
                          >
                            <path
            ...
            {checked && <Divider className={"mx-4"} />}
            {checked && children}
          </summary>
        </details>
      </div>
    ...
    <span
      className={
        "bg-destructive text-destructive-foreground px-6 py-4 rounded-md text-center"
      }
    >
