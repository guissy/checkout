...
const AliQrPage: React.FC = () => {
  ...
  const dataUrl = useQRCode({
    ...
    quality: 100,
    value:
      payInfo?.action?.qrCode ??
      payInfo?.action?.schemeUrl ??
      payInfo?.action?.applinkUrl ??
      "",
    width: 320,
  } as QRCodeOptions);
  ...
  const [svg] =
    getIcon(currentPay?.type as string, currentPay?.platformName as string) ??
    [];
  ...
  return (
    <div
      className={clsx(
        isMobile
          ? "pt-[68px] min-h-full flex flex-col items-center justify-between"
          : "grid place-items-center min-h-full",
      )}
    >
      <div
        className={clsx(
      ...
      >
        <div className={"text-center"}>
          <div
            className={
              "py-2.5 rounded-[12px] rounded-bl-none rounded-br-none flex items-center justify-center " +
          ...
          {loading || !dataUrl ? (
            <div
              className={
                "h-[70vw] max-h-[320px] flex justify-center items-center"
              }
            >
            ...
            <img
              src={dataUrl}
              className={
                "mx-auto w-[70vw] max-w-[320px] min-h-24 leading-[16rem] mt-2"
              }
              alt="QR Code"
            />
          )}
          <p className={"text-2xl font-bold text-[#133E67] mb-8"}>
            ...
            {isPhone() && (
              <button
                ...
                id={"pay-btn2"}
                className={clsx(
                  "flex space-x-3 items-center justify-center mx-auto mt-4 px-4 py-3 transition-colors",
