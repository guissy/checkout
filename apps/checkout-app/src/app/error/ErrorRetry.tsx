...
const ErrorRetry: React.FC<Props> = ({ detail, token, reference, isHome }) => {
  ...
  return (
    <div>
      <div className={"w-full h-[426px] bg-primary text-white flex items-center justify-center"}>
        <div>
          ...
          <div className={"text-center"}>
            ...
            <p className={"mt-4 text-white/70"}>
              {/* <Trans id={"failure.request_again"} message='Please request the transaction again' /> */}
              Please request the transaction again
            </p>
          </div>
        </div>
      </div>
      <div className={"mt-6 mb-10 text-center mx-6"}>
        {detail}
      </div>
