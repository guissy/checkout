...
const CardNumberInput: React.FC<Props> = ({
  ...
  const error = validateResult?.[key]
    || validateResult?.[cardFields.expiryYear?.key]
    || validateResult?.[cardFields.expiryMonth?.key]
    || validateResult?.[keyCvc];
  const [focus, setFocus] = React.useState<"cardNumber" | "expiryDate" | "cvv" | "">("");
  return (
    <div>
      <div className={clsx("relative flex-1", 'has-focus:z-20', focus === 'cardNumber' && "z-30")}>
        <Input name={key}
      ...
      <div className={clsx('flex w-full', 'has-focus:z-20', (focus === 'expiryDate' || focus === 'cvv') && "z-30")}>
        {cardFields.expiryYear &&
          ...
          />}
        {cardFields.cvv &&
          <div className={clsx("relative flex-1 -ml-px", 'has-focus:z-10', focus === "cvv" && "z-30")}>
            <Input name={keyCvc}
                   className={clsx('block w-full',
                   ...
                   onValueChange={(value) => {
                     setFormValue({ ...formValue, [keyCvc]: value } as FormValue,
                       [
