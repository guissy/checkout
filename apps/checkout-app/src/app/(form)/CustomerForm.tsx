...
const CustomerForm: React.FC<CustomFormerProps> = ({ countries, className, formValue, setFormValue, validateResult, countryCode, currentPay, optRegular }) => {
  const { supportedBankList } = currentPay || {};
  const regular = optRegular || currentPay?.regular;
  const allFields = getAllFields(regular, currentPay);
  const cardFields = {} as {
  ...
  const allFieldsSorted = Object.values(groupedFields).sort((a, b) => a[0].position && b[0].position ? a[0].position.localeCompare(b[0].position) : 0)
              ...
              <div className={"flex w-full relative"} key={items[0].position}>
                {items.map(({ key, label, regular, format, icon, isCard }, i) => {
                  if (key.endsWith('country'))
                    return (
                      <CountryInput
                        key={key}
                        name={key}
                        value={formValue?.[key as keyof typeof formValue] as string ?? ''}
                        onValueChange={(value) => {
                          setFormValue({ ...formValue, [key]: value } as FormValue, key as keyof FormValue);
                        }}
                        label={label}
                  ...
                  if (key.includes('expiryMonth') || key.includes('expiryYear'))
                    return (
                      key.includes('expiryMonth') ?
                        <ExpiryDateInput
                          key={key} formValue={formValue}
                          className={clsx('block w-full',
                            items.length > 1 && '[&_input]:rounded-none!',
                            items.length > 1 && i === 0 && '[&_input]:rounded-tl-md! [&_input]:rounded-bl-md!',
                            items.length > 1 && i === items.length - 1 && '[&_input]:rounded-tr-md! [&_input]:rounded-br-md!'
                          )}
                          setFormValue={setFormValue}
                          validateResult={validateResult}
                          label={isCard ? '' : label}
                  ...
                  return (
                    <div
                      key={key}
                      className={clsx("relative flex-1 has-focus:z-20", (focus === key) && "z-30", items.length > 1 && '-ml-px')}>
                      <Input name={key}
                             className={clsx('block w-full',
                               items.length > 1 && '[&_input]:rounded-none!',
                               items.length > 1 && i === 0 && '[&_input]:rounded-tl-md! [&_input]:rounded-bl-md!',
                             ...
                             required={true}
                             invalid={!!validateResult?.[key as keyof typeof validateResult]}
                             label={isCard ? '' : label}
                             value={formValue?.[key as keyof typeof formValue] as string ?? ''}
                             onValueChange={(value) => {
                               setFormValue({ ...formValue, [key]: value } as FormValue, key as keyof FormValue);
                             ...
                             onFocus={() => setFocus(key)}
                             autoFocus={i === 0 && j === 0}
                             autoComplete={key.includes("fullName") ? "cc-name" : undefined}
                             placeholder={'Please enter ' + label}
                      />
                      {icon && <img src={icon} alt="visa" className="h-6 w-auto absolute top-1 right-4"/>}
                      {isCard ? '' : <InputError>{validateResult?.[key as keyof typeof validateResult]}</InputError>}
                    </div>
                  )
                })}
              </div>
