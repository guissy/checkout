...
import fetchCountryInfoList, {
  type CountryInfo,
} from "../../api/fetchCountryInfoList";
...
const CountryInput: React.FC<Props> = ({
  ...
  React.useEffect(() => {
    if (open)
      lastPosRef.current?.parentElement?.scrollIntoView({ behavior: "smooth" });
  }, [open]);
  ...
  useEffect(() => {
    if (!!_countryCode && !dirty && dataSource.length > 0) {
      const countryNameEn = dataSource.find(
        (it) => it.iso2Code === _countryCode,
      )?.countryNameEn;
      if (countryNameEn && countryNameEn !== countryName) {
        setCountryName(countryNameEn);
        setCountryCode(_countryCode);

        // 防止无限循环：仅当当前 formValue 中的值与 _countryCode 不同时才更新
        const currentValue = formValue[props.name as keyof FormValue];
        if (currentValue !== _countryCode) {
          setFormValue(
            {
              [props.name as keyof FormValue]: _countryCode,
            } as unknown as FormValue,
            props.name as keyof FormValue,
          );
        }
      }
    }
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={clsx("flex w-full mb-2 relative", open && "z-40")}
        ref={wrapperRef}
      >
        <div className={"w-full bg-white"}>
          ...
          >
            ...
            >
              ...
              <span
                className={clsx(
                  "ml-1 inline-block transition-transform group-open:rotate-0",
                  open && "rotate-180",
                )}
              >
                ...
                {Array.isArray(dataSource) &&
                  dataSource
                    ?.filter((it) => {
                      ...
                      return (
                        it.countryNameEn.toLowerCase().startsWith(kw) ||
                        it.countryNameCn.toLowerCase().startsWith(kw)
                      );
                    })
                    ?.map((it, i) => (
                      <SelectItem
                        ...
                        onClick={() => {
                          ...
                          setFormValue(
                            {
                              ...formValue,
                              [props.name as keyof FormValue]: it.iso2Code,
                            },
                            props.name as keyof FormValue,
                          );
                          setOpen((b) => !b);
                      ...
                      >
                        <div
                          className={
                            "flex space-x-4 items-center text-md justify-between"
                          }
                        >
                          ...
                          <span
                            className={"text-base text-zinc-400 pr-2 hidden"}
                          >
                            {it.iso2Code}
                          </span>
                        </div>
      ...
      <InputError>
        {validateResult?.[props.name as keyof typeof validateResult]}
      </InputError>
