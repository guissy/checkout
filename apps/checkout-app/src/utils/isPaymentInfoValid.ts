...
function isPaymentInfoValid(paymentOrderInfo: PaymentOrderInfo | null, currentPay: PayMethod): boolean {
  ...
  const { holderName, firstName, lastName, shopperEmail } = paymentOrderInfo.paymentMethod;
  ...
  return isHolderOrFullNameValid && isShopperEmailValid && allFields.length === 0;
}
